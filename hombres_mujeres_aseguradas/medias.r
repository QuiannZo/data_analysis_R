...
# Calcular las medias de los salarios brutos (Salario_bruto)
media_salario_hombres_asegurados <- mean(hombres_asegurados$Salario_bruto, na.rm = TRUE)
media_salario_mujeres_aseguradas <- mean(mujeres_aseguradas$Salario_bruto, na.rm = TRUE)

# Crear un data frame con las medias para facilitar el gráfico
medias <- data.frame(
  Grupo = c("Hombres Asegurados", "Mujeres Aseguradas"),
  Media_Salario = c(media_salario_hombres_asegurados, media_salario_mujeres_aseguradas)
)

# Crear el gráfico de barras
ggplot(medias, aes(x = Grupo, y = Media_Salario, fill = Grupo)) +
  geom_bar(stat = "identity", width = 0.5) +
  labs(title = "Medias de salarios de hombres asegurados y mujeres aseguradas",
       x = "Grupo",
       y = "Media") +
  theme_minimal() +
...
ggsave("comparacion_salarios_hombres_mujeres_asegurados.png", width = 8, height = 6)
